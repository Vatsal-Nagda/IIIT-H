...
void log_server(string ip,string req_type){
	...
	fstream ser_log;
	ser_log.open("server.log",ios::app);
    ...
    int Month  = loc_time->tm_mon + 1;
  	...
  	if(req_type.compare("search")==0){
  		ser_log<<Day<<"-"<<Month<<"-"<<Year<<" "<<Hour<<"-"<<Min<<"-"<<Sec<<":"<<"Search request from "<<ip<<endl;
  	}
  	else if(req_type.compare("ack")==0){
  	  	ser_log<<Day<<"-"<<Month<<"-"<<Year<<" "<<Hour<<"-"<<Min<<"-"<<Sec<<":"<<"Share ack to "<<ip<<endl;	
  	}
  	else if(req_type.compare("search_resp")==0){
  		ser_log<<Day<<"-"<<Month<<"-"<<Year<<" "<<Hour<<"-"<<Min<<"-"<<Sec<<":"<<"Search response sent to "<<ip<<endl;	
  	}
  	else{
