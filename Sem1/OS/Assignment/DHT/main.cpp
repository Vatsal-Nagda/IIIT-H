...
void sendTableEntries(int sockfd,char recvBuff[]){
	...
	cout<<"Receiver Buffer : "<<recvBuff<<endl;
...
void checkInLeafSet(char recvBuff[],bool add=true){
	...
	if (check_id>atoi(nodeId))
		leftSet=false;
	if(leftSet){
		for(i=0;i<2;i++){
			if(atoi(LeftLeafSet[i].nodeid)==check_id){
				inLeafSet=true;
				/*cout<<atoi(LeftLeafSet[i].nodeid)<<endl;
				cout<<check_id<<endl;*/
			}
		}
	}
	...
	if(add){
		...
		if(leftSet && leftLeafCnt<2){
			LeftLeafSet[leftLeafCnt++] = received_packet;
		}
		else if(!leftSet && rightLeafCnt<2){
			RightLeafSet[rightLeafCnt++] = received_packet;
		}
	}
}
