...
class DigitClassify_NeuralNetwork(object):
    ...
    def Sensitivity(self,input_x,target):
        ...
        # for j in xrange(nH):
        # 	for k in xrange(c):
        # self.del_j = np.dot(del_k,weight_kj)
        # for j in xrange(nH):
        # 	self.del_j[j] = self.del_j[j] * derivate_sigmoid(self.net_j[j])
...
Digit_net = DigitClassify_NeuralNetwork([64,200,3],0.06)
