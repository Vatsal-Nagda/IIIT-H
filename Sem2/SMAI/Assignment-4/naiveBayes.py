...
for i in range(30):
	...
	for train_index, test_index in kf.split(data):
		...
		probLess = math.log(1 + numLess/trainData.shape[0])  #a-priori probability (log probabilities)
		probMore = math.log(1 + numMore/trainData.shape[0])
		classLessDict = {}
		classMoreDict = {}
		...
		for i in range(testData.shape[0]):
			...
			for attribute in categorical_attributes[:-1]:
				if X[attribute].iloc[0] in classLessDict[attribute].keys():
					posteriorLess += math.log(1 + classLessDict[attribute][X[attribute].iloc[0]])
				if X[attribute].iloc[0] in classMoreDict[attribute].keys():
					posteriorMore += math.log(1 + classMoreDict[attribute][X[attribute].iloc[0]])
